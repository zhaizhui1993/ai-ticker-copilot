...
> 来源：原方案 §5.3；v1.2 实施期扩充批次：10 → 26 条

**种子事件清单（26 条）。量化数据两种来源**：tag 含"公开数据待复核"= 来自公开报道的硬数字，待 `backfill_events.py` 实测复核；tag 含"2026-09复盘实测" = 本仓实测。正向事件 8 条配平（防类比永远偏空）。
...
| 2026-07-ai-valuation-pullback | AI 变现疑虑+收益率上行的半导体集中回调 | tech | 负 | **深回调买点原型**（指数未破位、VIX 20.7），七标的实测 |
| **公司事件（4）** | | | | |
