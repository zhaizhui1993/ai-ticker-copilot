...
> **职责**：四维规则评分（宏观/事件/产业/公司）、技术面分层指标体系、体制层分级约束、公司面短板门槛、加权汇总与信号分档——全系统可解释、可复现的分数锚点，LLM 不可用时仍可独立产出。
...
> **内容来源**：原方案 §6.1（含 §6.1.1 分层指标体系与汇总分档；v1.1 硬约束 → v1.2 分级约束改造）
...
Scorer 是**纯函数**（同输入同输出、无 IO、无 LLM）；体制层分级约束与公司面短板门槛只在 Engine 施加，Scorer 不感知（契约见 [10-module-contracts.md §10.2③](../10-module-contracts.md)）。
...
| [engine.md](engine.md) | scoring/engine.py | 加权汇总、信号分档、体制层分级约束、公司面短板门槛 | — |
