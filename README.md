...
- **历史事件类比**（核心特色）：当前事件与历史事件库（贸易摩擦、加息周期、芯片管制、DeepSeek 冲击等，种子 43 条）做相似度匹配，用历史实测数据（最大回撤/达底天数/恢复天数）研判当前影响；**状态条件化（v1.3）**：历史样本携带事件前状态（乖离率/距 52 周高，`backfill_pre_state.py` 回填），类比按"事件相似 × 状态可比"双维度判断、冲击按拥挤度缺口调节（×0.5~×1.5）——同样的管制打在新高与深跌位置结论不同；库含 fizzled 对照（预期冲击未兑现）与 pre-2018 机制类比，防"全是深冲击样本"高估影响；事件库随使用自动沉淀增长
- **技术面分层指标体系**：体制层（指数 vs MA200 + VIX，只做门槛）→ 趋势层（MA50 斜率）→ 择时层（MA20 回踩 / RSI14 / ATR14 / 量比）；伤害度量以"52 周回撤 + 乖离率"为主
- **体制层分级约束**：指数环境破位时档位压至"中性偏多"、信号上限=观望偏加仓（小仓）并输出**仓位上限系数 0.3**（v1.2：不再一刀切观望，保留恐慌底部的小仓试探空间）；另设**公司面短板门槛**（公司分 <35 时档位封顶，基本面恶化不被其他维度平均掉）
...
- **降级优先于失败**：任一数据源失效计中性分并标注，主流程不中断；评分子腿数据缺失时显式重归一化剩余权重（不塞中性 50）
- **信号评测闭环**：`scripts/backtest_signals.py`——①信号前瞻研究（5/20/60d 收益与 MAE，**对全池等权同期基准取超额**，按信号×体制破位分组）；②LLM 偏离质量追踪（偏离组 vs 遵守组 + 体制封顶违背计数）；③校准曲线（置信度 vs 实际胜率）；④`--full` 月度全量评估**追加式存档**（`data/eval/monthly-YYYY-MM.md` 只增不覆盖，调度器每月 1 日自动生成）——固定指标集固定口径，历史报告不可回改，防自欺
...
[3] 四维评分：宏观 / 事件(类比+情绪) / 产业(篮子强弱) / 公司(基本面+技术面)
    [4] 体制门槛：指数破位 → 档位压至中性偏多 + 仓位上限系数 0.3（环境只做开关，不进加权）
