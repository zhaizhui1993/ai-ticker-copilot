...
- **准实时事件管道**：新闻 RSS + Google News 主题流 15~30 分钟轮询，增量去重 → 预过滤 → LLM 结构化为四类事件（宏观政策/国际热点/行业/企业）
- **多源免费数据**：yfinance（行情/财报/指数）+ FRED（宏观七系列）+ RSS + X 爬虫（Playwright，防风控）
- **经济日历**：FOMC / CPI / 财报日预置，"预告 → 实时跟进 → 历史类比"闭环
- **对话式研究助理**：基于系统数据（四维分/指标/事件类比/持仓）的多轮对话，可追问报告理由、质疑结论、做情景推演（"如果跌破 MA50？"）；助理只引用系统已采集的数据，缺失直说不编造
...
```
[1] 采集（两条独立管道）
    事件管道（15~30min 轮询）：多源抓取 → 去重 → 预过滤 → LLM 结构化 → 入库+类比+提醒
    数据管道（低频）：yfinance 行情/财报 + FRED 宏观 + X 帖子
[2] 事件匹配：当前事件 → 硬检索历史库 top-5 → LLM 精排(相似度/方向/强度) → 类比结论
...
uv run playwright install chromium   # 仅爬 X 需要（约 150MB，可跳过）
...
| `uv run python scripts/backfill_events.py --event <event_id>` | 历史事件量化回填（实测回撤/达底/恢复） |
| `uv run python scripts/analyze.py --ticker NVDA` | 端到端分析（信号/理由/风险/免责声明） |
| `uv run python scripts/export_x_cookie.py` | 有头浏览器登录 X，导出登录态（首次爬 X 前） |
| `uv run python scripts/crawl_x.py` | 手动抓 X 帖子并入库 |
...
| `DB_*` | MySQL 连接 |
| `SCHEDULER_ENABLED` / `EVENT_POLL_INTERVAL_MIN` | 调度开关 / 事件轮询间隔（15~30min，默认 20） |
...
├── collectors/             # 采集层：market/macro/news/x_crawler/mock + TTL 缓存
├── events_lib/             # 历史事件库：种子 YAML + 两级匹配 + 联动回填算法
├── scoring/                # 四维评分引擎（P5）
├── analyzer/               # LLM 出口 + run_analysis 编排（P6）
├── storage/                # MySQL 连接池 + repository（唯一读写口）
├── web/                    # FastAPI + 前端面板（P8）
├── scheduler.py            # APScheduler 三类任务（P9）
├── scripts/                # CLI 工具（每数据源可独立验证）
├── docs/                   # 模块化技术方案（49 篇）
└── tests/                  # pytest（DB 测试无实例时自动 skip）
```
...
| P7 | X 爬虫（登录态导出/防风控/情绪打标管线） | ✅ 完成 |
| P8 | Web 面板（五页签/K线/vendor 本地化/409 锁） | ✅ 完成 |
| P9 | 调度器四任务/通知/日历/自动沉淀 | ✅ 完成 |

> 单元测试 48 passed；DB 相关 6 项测试需配置 MySQL 凭据后自动启用。
> 真实数据联调 checklist：① `.env` 填 `DB_*`（本机 3306 已有实例）；② 填 `LLM_*`
> 与 `FRED_API_KEY`；③ 雅虎限频恢复后 `scripts/refresh_data.py --scope market`；
> ④ 首次爬 X 前 `scripts/export_x_cookie.py` 登录一次。
