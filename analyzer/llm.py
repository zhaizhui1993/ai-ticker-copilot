"""LLM 层占位实现（P3）：规则版事件抽取。

P6 阶段将替换为真实 LLM（供应商无关，OpenAI 兼容，docs/06-analyzer/provider.md），
接口签名与本文件保持一致（契约 §10.2④），管道与测试零改动切换。

当前实现：关键词规则猜测 scope/category，够事件管道联调与 MOCK 全链路使用，
明确标注 extraction="rule-based"（低置信，未经 LLM 校验）。
"""
...
def extract_events(
    ...
    """规则版事件抽取（P6 替换为 LLM 批量单次调用，签名不变）。"""
